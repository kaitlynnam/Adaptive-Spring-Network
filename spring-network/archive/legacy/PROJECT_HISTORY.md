...
> **Current default fan:** `topologies/internal_fan_20_spring_model.json` with 20 springs. The original `internal_fan_model.json` is retained as a legacy 40-spring comparison. Default fan training now writes `models/adaptive_trained_internal_fan_20spring_model.npz`.
...
The checked-in artifacts predate the classified-arbitrary, profile-aware input pipeline and must be retrained before evaluation. New models use the 70-input `profile_motion_torque_window` format: 10 normalized profile-knot values plus a 10-sample causal window of six motion/realized-torque signals.
...
The current project uses one profile set everywhere: arbitrary five-knot piecewise-linear torque profiles. All five torque values are sampled independently before classification. Training, held-out testing, batch evaluation, model comparison, and named single-profile evaluation share this generator.
...
```text
5 torque-angle points (this project's explicit resolution choice)
...
All five torque values are sampled independently in +/-115 N*m. The generator does not impose a restoring-spring, linear, or sinusoidal target. It classifies the resulting arbitrary curves using an explicit shape score:

```text
35% normalized error from the best-fit straight line
30% excess total variation beyond a monotonic curve
20% normalized variation among segment slopes
15% fraction of segment-direction reversals
```

The score is calibrated so a straight line scores below a smooth sampled sinusoid, which scores below a repeatedly reversing zigzag. After scoring, the lowest third becomes `flat_terrain`, the middle third becomes `mixed_terrain`, and the highest third becomes `rough_terrain`. Training and held-out populations are generated and classified independently. These are relative torque-shape categories retained for reporting convenience, not terrain labels measured from a robot. Motion parameters do not affect classification.
...
New training gives the MLP the commanded static torque-angle curve plus ten causal samples of six signals:
...
commanded profile:
    5 normalized knot angles + 5 normalized knot torques = 10 inputs
...
At each timestep the model predicts stiffness before the current torque values are added to history. The mechanics solver then computes realized spring torque, residual motor torque is derived, and those three torque values become available only to the next prediction. The first torque-history window is zero because nothing has happened yet. The commanded static profile is known, but current/future motion samples and a separate future torque trace are not model inputs.

The complete commanded torque-angle profile is intentionally supplied because arbitrary profiles are independent of motion and cannot otherwise be identified from the same motion history. Evaluation requires the same five knots and uses the `profile_motion_torque_window` format. Current/future motion samples are never inputs.
...
By default it runs one balanced randomized batch using the same classified arbitrary generator as training:
...
The generated profiles vary independent piecewise-linear torque knots, motion amplitude, motion frequency, phase, bumps, and smoothed noise. The torque curves are ranked by shape roughness and divided into equal thirds. The evaluator reports whole-batch averages followed by separate `flat_terrain`, `mixed_terrain`, and `rough_terrain` shape-class averages for offload, energy saved, mean absolute torque error, and maximum absolute torque error.
...
stiffness mode = adaptive_trained_internal_fan_model.npz
                 (70 profile/motion/torque-history inputs, 40 outputs after retraining)
node handling  = quasi-static internal-node relaxation enabled
evaluation     = 300 classified arbitrary piecewise-linear trajectories
...
After retraining, this selects the `fan` preset by default and uses its 70-input profile-aware causal model. The evaluator predicts stiffness sequentially, evaluates the balanced arbitrary-profile batch, prints overall plus relative flat/mixed/rough shape statistics, and saves:
...
By default this generates 6,000 arbitrary training trajectories (2,000 for each of three relative shape classes) and 1,200 independently generated held-out trajectories (400 per class). It then trains the adaptive stiffness model and writes:
...
Training uses relaxed-node mechanics for its torque basis. It receives the commanded five-knot torque-angle profile, current/past motion, and previously realized target, spring, and residual motor torque. It receives no future motion samples, terrain label, or roughness score. The default MLP has 256 hidden units and predicts one stiffness per spring in the selected topology, in the 1 to 800 N/m range. Training rolls forward in time so each prediction can use the torque consequences of earlier stiffness decisions. Final train/test metrics replay the model sequentially through the full relaxed mechanics.

Training uses gradient descent through PyTorch automatic differentiation. Each iteration performs a causal forward rollout, computes the differentiable loss, calls `loss.backward()` to obtain gradients for every MLP weight and bias, and calls the Adam optimizer's `step()` method to update those parameters. Adam is an adaptive stochastic-gradient method; the default run uses 5,000 full-dataset optimization iterations with learning rate 0.01.

The differentiable loss combines mean squared torque error, a penalty for moving stiffness far from its topology initialization, and an energy/offload term using the same bidirectional energy burden as evaluation. It penalizes electrical motoring draw plus the portion of braking energy that is not regenerated. The convergence plot shows training RMSE, loss, the offload surrogate, marks the best-loss iteration, and overlays the fixed-stiffness baseline train/test RMSE for scale.
