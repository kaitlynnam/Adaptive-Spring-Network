...
def get_robot(env: Any) -> Any:
    ...
    if "robot" in scene:
        return scene["robot"]
    if "articulation" in scene:
        return scene["articulation"]
    keys = list(scene.keys()) if hasattr(scene, "keys") else []
    raise KeyError(f"Could not find robot articulation in scene. Available scene keys: {keys}")
